...
class Slider():
    ...
    def __init__(self, center, size, initial_val, min_val, max_val, label):
        self.center = center
        self.size = size
    ...
    def DisplayLabel(self, screen):
        RenderText(screen, self.container_rect.centerx, self.top_pos-35, self.label, "black", 32, center=True)
    

    #Display the value next to the slider
    def DisplayValue(self, screen):
        RenderText(screen, self.left_pos + self.size[0] + 50, self.container_rect.centery, "{:.2f}".format(self.GetValue()), "black", 22, center=True)